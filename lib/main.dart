...
class Army {
  ...
  static Army fromJson(Map<String,dynamic> j)=>Army(soldiers:j['soldiers']??0,tanks:j['tanks']??0,artillery:j['artillery']??0,airDefense:j['airDefense']??0,aircraft:j['aircraft']??0,helicopters:j['helicopters']??0,drones:j['drones']??0,recon:j['recon']??0,navy:j['navy']??0);
}
...
class StrategicSite {
  ...
  static StrategicSite fromJson(Map<String,dynamic> j)=>StrategicSite(id:j['id']?.toString()??'',countryId:j['countryId']?.toString()??'',name:j['name']?.toString()??'منشأة',kind:j['kind']?.toString()??'factory',offset:Offset((j['dx']??0).toDouble(),(j['dy']??0).toDouble()),health:(j['health']??100).toDouble(),level:j['level']??1);
}
...
class FieldArmy {
  ...
  static FieldArmy fromJson(Map<String,dynamic> j)=>FieldArmy(id:j['id']?.toString()??'',name:j['name']?.toString()??'جيش ميداني',owner:j['owner']?.toString()??'',location:j['location']?.toString()??'',soldiers:j['soldiers']??0,tanks:j['tanks']??0,artillery:j['artillery']??0,airDefense:j['airDefense']??0,aircraft:j['aircraft']??0,helicopters:j['helicopters']??0,drones:j['drones']??0,recon:j['recon']??0,supply:(j['supply']??70).toDouble(),morale:(j['morale']??70).toDouble());
}
...
class FrontBattle {
  ...
  static FrontBattle fromJson(Map<String,dynamic> j)=>FrontBattle(id:j['id']?.toString()??'',source:j['source']?.toString()??'',target:j['target']?.toString()??'',attacker:j['attacker']?.toString()??'',defender:j['defender']?.toString()??'',plan:j['plan']?.toString()??'balanced',status:j['status']?.toString()??'active',progress:(j['progress']??0).toDouble(),round:j['round']??0,startedTurn:j['startedTurn']??1,lastResolvedTurn:j['lastResolvedTurn']??0,naval:j['naval']==true,encircled:j['encircled']==true,airControl:(j['airControl']??0).toDouble(),navalControl:(j['navalControl']??0).toDouble(),targetProvinceId:j['targetProvinceId']?.toString(),attackerArmyIds:List<String>.from(j['attackerArmyIds']??const []),defenderArmyIds:List<String>.from(j['defenderArmyIds']??const []),history:(j['history'] is List)?(j['history'] as List).whereType<Map>().map((x)=>Map<String,dynamic>.from(x)).toList():<Map<String,dynamic>>[]);
}
...
class Province {
  ...
  String controller;
  double population,resistance,lat,lng,infrastructure,supply;
  int garrison,fortLevel,trainingCamps;
  Province({required this.id,required this.countryId,required this.name,String? nameEn,String? code,required this.controller,this.population=1,this.resistance=0,this.lat=0,this.lng=0,this.infrastructure=60,this.supply=70,this.garrison=0,this.fortLevel=0,this.trainingCamps=0}):nameEn=nameEn??name,code=code??id;
  double get defensePower=>garrison*(1+fortLevel*.22)*(0.72+infrastructure/180)*(0.7+supply/230);
  Map<String,dynamic> toJson()=>{'id':id,'countryId':countryId,'name':name,'nameEn':nameEn,'code':code,'controller':controller,'population':population,'resistance':resistance,'lat':lat,'lng':lng,'infrastructure':infrastructure,'supply':supply,'garrison':garrison,'fortLevel':fortLevel,'trainingCamps':trainingCamps};
  static Province fromJson(Map<String,dynamic> j)=>Province(id:j['id']?.toString()??'',countryId:j['countryId']?.toString()??'',name:j['name']?.toString()??'',nameEn:j['nameEn']?.toString(),code:j['code']?.toString(),controller:j['controller']?.toString()??'',population:(j['population']??1).toDouble(),resistance:(j['resistance']??0).toDouble(),lat:(j['lat']??0).toDouble(),lng:(j['lng']??0).toDouble(),infrastructure:(j['infrastructure']??60).toDouble(),supply:(j['supply']??70).toDouble(),garrison:j['garrison']??0,fortLevel:j['fortLevel']??0,trainingCamps:j['trainingCamps']??0);
}
...
class GameState extends ChangeNotifier {
  ...
  Future<void> initializeWorldData()async{
    ...
    try{
      ...
      await _loadProvinceGeometry();worldDataLoaded=true;_log(tr('تم تحميل عالم المحافظات: ${countries.length} دولة و${provinces.length} محافظة/ولاية.','World loaded: ${countries.length} countries and ${provinces.length} provinces/states.'));notifyListeners();
  ...
  Future<void> choose(String id) async {
    if(multiplayer?.isConnected==true){
      final ok=await multiplayer!.claimCountry(id);
      if(!ok){_log('لا يمكن اختيار ${countries[id]!.name}: اختارها لاعب آخر في هذه الغرفة.');return;}
    }
    humanCountry=id;winnerController=null;selected=id;countries[id]!.controller='P:$id';
    logs.insert(0,'بدأت اللعب بـ ${countries[id]!.name}.');_sync();notifyListeners();
  }
  bool isCountryTaken(String id){
    if(multiplayer?.isConnected!=true)return false;
    final owner=onlineCountryOwners[id];
    return owner!=null&&owner!=multiplayer!.playerId;
  }
  void select(String id){selected=id;notifyListeners();}
  bool isMine(String id)=>humanCountry!=null&&countries[id]?.controller=='P:$humanCountry';
  String diplomaticIdentity(String id){final c=countries[id];if(c==null)return id;return (c.controller.startsWith('P:')||c.controller.startsWith('AI:'))?controllerRoot(c.controller):id;}
  bool areAllies(String a,String b)=>alliances.contains(_pair(diplomaticIdentity(a),diplomaticIdentity(b)));
  String _pair(String a,String b){final pair=[a,b]..sort();return pair.join('|');}

  void toggleAlliance(String other){
    if(multiplayer?.isConnected==true){if(areAllies(humanCountry!,other))multiplayer!.sendGameAction('endAlliance',target:other);else multiplayer!.sendGameAction('offerAlliance',target:other);return;}
    if(humanCountry==null||other==humanCountry)return; final p=_pair(diplomaticIdentity(humanCountry!),diplomaticIdentity(other));
    if(alliances.contains(p)){alliances.remove(p);logs.insert(0,'تم إنهاء التحالف مع ${countries[other]!.name}.');}
    else {alliances.add(p);logs.insert(0,'تم توقيع تحالف مع ${countries[other]!.name}.');}
    _sync();notifyListeners();
  }

  void respondAlliance(String offerId,bool accept){
    if(multiplayer?.isConnected==true){multiplayer!.sendGameAction('respondAlliance',value:'$offerId|${accept?'accept':'reject'}');return;}
  }
  ...
  bool isSupplyConnected(String countryId){
    if(humanCountry==null)return false;
    final controller='P:$humanCountry';
    if(countries[countryId]?.controller!=controller)return false;
    final root=me.id;if(countryId==root)return true;
    final seen=<String>{root};final queue=<String>[root];
    while(queue.isNotEmpty){final cur=queue.removeAt(0);for(final n in _supplyNeighbors(controller,cur)){if(seen.contains(n)||countries[n]?.controller!=controller)continue;if(n==countryId)return true;seen.add(n);queue.add(n);}}
    return false;
  }
  ...
  double get aiMultiplier=>aiDifficulty=='hard'?1.18:aiDifficulty=='easy'?.86:1.0;
  String get aiDifficultyLabel=>aiDifficulty=='hard'?'صعب':aiDifficulty=='easy'?'سهل':'متوازن';
  ...
  double attackPlanPower(String p)=>p=='breakthrough'?1.22:p=='cautious'?.90:1.0;
  double attackPlanCost(String p)=>p=='breakthrough'?1.28:p=='cautious'?.86:1.0;
  ...
  double intelConfidence(Country target){if(isMine(target.id))return 1;final raw=(me.intelligence+me.techIntel*11+me.army.recon*2.2+me.army.drones*1.1-target.intelligence*.28)/100;return raw.clamp(.22,.95).toDouble();}
  ...
  double _airAssets(Country c,List<String> ids){var total=c.army.aircraft*2+c.army.drones+c.army.helicopters*.6;for(final id in ids){for(final a in fieldArmies){if(a.id==id){total+=a.aircraft*2+a.drones+a.helicopters*.6;break;}}}return total;}
  ...
  ({double attacker,double defender}) _battleLosses(double ratio,bool wonRound){final r=max(.05,ratio);return (attacker:((wonRound?.06:.10)+(r<1?(1-r)*.12:0)).clamp(.035,.24).toDouble(),defender:((wonRound?.13:.08)+(r>1?(r-1)*.07:0)).clamp(.04,.30).toDouble());}
  ...
  void moveFieldArmy(String armyId,String destination){
    ...
    if(me.stock.fuel<2){_noRes();return;}if(!_spendCommand(1))return;me.stock.fuel-=2;a.location=destination;a.supply=max(0,a.supply-5);_log('${a.name} تحرك إلى ${to.name}.');notifyListeners();
  }
  ...
  void strikeSite(String siteId){
    ...
    final target=countries[site.countryId]!;final c=me;
    ...
    final chance=(.48+c.army.drones*.025+c.intelligence/320-target.army.airDefense*.022).clamp(.18,.92);
  ...
  void strategicStrike(String targetId,String kind){
    ...
    if(kind=='air'){
      ...
      if(!_spendCommand(2))return;c.stock.fuel-=7;c.stock.money-=8;final success=.45+c.army.drones*.025+c.intelligence/300-target.army.airDefense*.025;
      ...
      if(c.army.navy<1||c.stock.fuel<5)return _noRes();if(!_spendCommand(2))return;c.stock.fuel-=5;target.stock.fuel=max(0,target.stock.fuel-10);target.stock.food=max(0,target.stock.food-7);target.approval=max(0,target.approval-3);_log('حصار بحري على ${target.name}: تراجعت واردات الغذاء والوقود.');
    }
    _sync();notifyListeners();
  }
  ...
  void nextTurn(){
    ...
    for(final p in provinces){final owner=countries[p.countryId];if(owner==null)continue;final connected=_provinceSupplyConnected(p);if(!connected){p.supply=max(0,p.supply-7);p.infrastructure=max(25,p.infrastructure-.4);}else{p.supply=min(100,p.supply+2.2+owner.techLogistics*.35);if(p.supply>60)p.infrastructure=min(95,p.infrastructure+.18);}if(p.resistance>70){p.infrastructure=max(20,p.infrastructure-.5);p.supply=max(0,p.supply-2);}if(p.garrison>0&&p.supply<25)p.garrison=max(0,p.garrison-(rng.nextDouble()<.15?1:0));}
    for(final entry in bases.entries){final ctrl=entry.key,base=entry.value;if(base.controller!=ctrl)continue;final owned=countries.values.where((c)=>c.controller==ctrl);final force=owned.fold<double>(0,(n,c)=>n+combatPower(base,c.army))+fieldArmies.where((a)=>a.owner==ctrl).fold<double>(0,(n,a)=>n+a.power);final upkeep=Resources(food:force*.018,fuel:force*.012,money:force*.055);if(base.stock.canPay(upkeep))base.stock.pay(upkeep);else{base.supply=max(0,base.supply-9);base.morale=max(0,base.morale-7);base.approval=max(0,base.approval-3);_log('${controllerRoot(ctrl)} يعاني عجزًا في صيانة القوات.');}base.warExhaustion=max(0,base.warExhaustion-2.5);if(base.warExhaustion>50){base.approval=max(0,base.approval-1.5);base.morale=max(0,base.morale-2);}if(base.warExhaustion>75){base.stability=max(0,base.stability-2);base.approval=max(0,base.approval-2);}base.commandPoints=(gamePace=='rapid'?8:gamePace=='grand'?5:6)+base.techLogistics;base.recruitedThisTurn=0;}
    ...
    try{
      ...
      _ws!.listen((raw) async {
        ...
        if(type=='joined'){
          ...
          revision=(m['revision']??0) as int;hostPlayerId=m['hostPlayerId']?.toString()??'';roomStatus=m['status']?.toString()??'';serverVersion=m['serverVersion']?.toString()??'';roomPace=m['pace']?.toString()??roomPace;turnDeadline=(m['turnDeadline'] as num?)?.toInt();serverHasSnapshot=m['snapshot']!=null;
      ...
      _ws!.add(jsonEncode({'type':'join','protocol':kProtocolVersion,'clientVersion':kAppVersion,'mode':mode,'password':password,'room':roomCode,'playerId':playerId,'resumeToken':resumeToken,'name':playerName,'installId':installationId}));
...
class StartScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){
    return Scaffold(body:SafeArea(child:Padding(padding:const EdgeInsets.all(20),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
      ...
      Text(game.tr('v1.4 • خريطة محافظات فعلية + استخبارات + مدة مباراة مرنة','v1.4 • Real province map + intelligence + flexible match pace'),style:const TextStyle(fontSize:12,color:Colors.white54)),
      ...
      body:ListView(
        ...
        children:[
          Card(child:Padding(padding:const EdgeInsets.all(14),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
            ...
            Text('السيرفر $lobbyServerVersion • الاتصال مشفّر: $encryptedLabel'),
            const SizedBox(height:8),
...
Future<void> _showPrivacy(BuildContext context,GameState game)async{
  await showModalBottomSheet(context:context,isScrollControlled:true,builder:(ctx)=>SafeArea(child:Padding(padding:const EdgeInsets.all(20),child:SingleChildScrollView(child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,mainAxisSize:MainAxisSize.min,children:[
    const Text('الخصوصية وبيانات اللعبة',style:TextStyle(fontSize:22,fontWeight:FontWeight.bold)),const SizedBox(height:10),const Text('World Dominion لا يحتاج حساب بريد إلكتروني في هذه النسخة. عند اللعب أونلاين يعالج السيرفر معرف ضيف عشوائيًا وHash لمعرف تثبيت مستعار لأغراض الاستعادة ومنع الإساءة، إضافة إلى اسم العرض والدولة المختارة وحالة المباراة. لا توجد مكتبة إعلانات أو تحليلات مضافة في هذا الإصدار.'),const SizedBox(height:10),Text('الإصدار: $kAppVersion • بروتوكول: $kProtocolVersion'),if(kSupportEmail.isNotEmpty)Text('الدعم: $kSupportEmail'),const SizedBox(height:12),if(kEffectivePrivacyPolicyUrl.isNotEmpty)OutlinedButton.icon(onPressed:()=>launchUrl(Uri.parse(kEffectivePrivacyPolicyUrl),mode:LaunchMode.externalApplication),icon:const Icon(Icons.open_in_new),label:const Text('فتح سياسة الخصوصية الكاملة')),OutlinedButton.icon(onPressed:()=>showLicensePage(context:context,applicationName:'World Dominion',applicationVersion:kAppVersion),icon:const Icon(Icons.description_outlined),label:const Text('تراخيص البرمجيات المفتوحة')),
...
class _GameScreenState extends State<GameScreen>{
  ...
  @override Widget build(BuildContext context){final g=widget.game;final m=g.multiplayer;final online=m?.isRunning==true;
    return Scaffold(appBar:AppBar(title:Text('${g.me.name} • ${g.tr('الدور','Turn')} ${g.turn} • ${g.gamePaceLabel}'),actions:[
      ...
      if(g.winnerController!=null)Container(width:double.infinity,padding:const EdgeInsets.all(10),color:Colors.amber.withOpacity(.16),child:Text('🏆 انتهت الحرب — الفائز: ${g.winnerName} • ${g.victoryType=='hegemony'?'نصر الهيمنة':'السيطرة الكاملة'}',textAlign:TextAlign.center,style:const TextStyle(fontWeight:FontWeight.bold))),
      if(g.eliminated&&g.winnerController==null)Container(width:double.infinity,padding:const EdgeInsets.all(10),color:Colors.redAccent.withOpacity(.14),child:const Text('خسرت آخر إقليم. يمكنك متابعة الحرب كمتفرج.',textAlign:TextAlign.center,style:TextStyle(fontWeight:FontWeight.bold))),
      if(!g.eliminated)_CommandStrip(game:g),
      Expanded(child:IndexedStack(index:tab,children:[PoliticalMapTab(game:g),StateTab(game:g),MilitaryTab(game:g),EconomyTab(game:g),DiplomacyTab(game:g),StrategyTab(game:g),InfrastructureTab(game:g),LogTab(game:g)]))
    ]),bottomNavigationBar:NavigationBar(labelBehavior:NavigationDestinationLabelBehavior.onlyShowSelected,selectedIndex:tab,onDestinationSelected:(i)=>setState(()=>tab=i),destinations:[NavigationDestination(icon:const Icon(Icons.public),label:g.tr('الخريطة','Map')),NavigationDestination(icon:const Icon(Icons.account_balance),label:g.tr('الدولة','State')),NavigationDestination(icon:const Icon(Icons.shield),label:g.tr('الجيش','Military')),NavigationDestination(icon:const Icon(Icons.factory),label:g.tr('الاقتصاد','Economy')),NavigationDestination(icon:const Icon(Icons.handshake),label:g.tr('الدبلوماسية','Diplomacy')),NavigationDestination(icon:const Icon(Icons.radar),label:g.tr('استراتيجية','Strategy')),NavigationDestination(icon:const Icon(Icons.location_city),label:g.tr('المدن','Cities')),NavigationDestination(icon:const Icon(Icons.history),label:g.tr('السجل','Log'))]));
  }
}
...
class _CommandStrip extends StatelessWidget{
  ...
  @override Widget build(BuildContext context){final c=game.me,territories=game.countries.values.where((x)=>x.controller==c.controller).length;return Container(height:52,decoration:BoxDecoration(color:Colors.white.withOpacity(.035),border:Border(bottom:BorderSide(color:Colors.white.withOpacity(.07)))),child:ListView(scrollDirection:Axis.horizontal,padding:const EdgeInsets.symmetric(horizontal:8,vertical:7),children:[
...
Widget _miniStatus(IconData icon,String text)=>Container(margin:const EdgeInsets.only(right:7),padding:const EdgeInsets.symmetric(horizontal:10),decoration:BoxDecoration(color:Colors.white.withOpacity(.055),borderRadius:BorderRadius.circular(20)),child:Row(children:[Icon(icon,size:16),const SizedBox(width:5),Text(text,style:const TextStyle(fontSize:12,fontWeight:FontWeight.w600))]));
...
Color _controllerColor(GameState game,Country c){
  ...
  if(game.isOccupied(c)){final h=c.controller.codeUnits.fold<int>(0,(a,b)=>a+b);return Colors.primaries[h%Colors.primaries.length].shade300.withOpacity(.82);}
  return Colors.orangeAccent.withOpacity(.72);
}
...
class _ProvinceMapSheet extends StatelessWidget{
  ...
  @override Widget build(BuildContext context){final country=game.countries[countryId]!,selected=game.provinceById(game.selectedProvinceId),shapes=game.provinceShapesByCountry[countryId]??const <ProvinceShape>[];return Padding(padding:const EdgeInsets.all(12),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
    ...
    Text(shapes.isEmpty?game.tr('لا توجد هندسة GIS محلية في ملف التطوير الحالي؛ GitHub Actions يبنيها تلقائيًا من Natural Earth.','No local GIS geometry in this development bundle; GitHub Actions builds it automatically from Natural Earth.'):game.tr('${shapes.length} تقسيم إداري مرسوم وقابل للنقر.','${shapes.length} administrative polygons are drawn and tappable.'),style:const TextStyle(fontSize:12,color:Colors.white60)),const SizedBox(height:8),
    ...
    const SizedBox(height:8),if(selected!=null&&selected.countryId==countryId)Expanded(flex:3,child:SingleChildScrollView(child:Card(child:Padding(padding:const EdgeInsets.all(12),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
      ...
      Text('${game.tr('الحامية','Garrison')} ${selected.garrison} • ${game.tr('التحصين','Fortification')} ${selected.fortLevel} • ${game.tr('قوة الدفاع','Defense power')} ${selected.defensePower.toStringAsFixed(1)}'),const SizedBox(height:10),
      if(selected.controller==game.me.controller)Wrap(spacing:8,runSpacing:8,children:[FilledButton.tonalIcon(onPressed:()=>game.buildProvinceDefense(selected.id),icon:const Icon(Icons.fort),label:Text(game.tr('تقوية الدفاع','Fortify'))),OutlinedButton.icon(onPressed:()=>game.reinforceProvince(selected.id),icon:const Icon(Icons.group_add),label:Text(game.tr('إرسال 2 مشاة','Send 2 infantry')))])
...
class _ProvincePainter extends CustomPainter{final GameState game;final String countryId;final _GeoBounds bounds;_ProvincePainter({required this.game,required this.countryId,required this.bounds});@override void paint(Canvas canvas,Size size){canvas.drawRect(Offset.zero&size,Paint()..color=const Color(0xFF071923));final shapes=game.provinceShapesByCountry[countryId]??const <ProvinceShape>[];for(final shape in shapes){final province=game.provinceForShape(shape),selected=province?.id==game.selectedProvinceId,mine=province?.controller==game.me.controller;final fill=Paint()..color=(mine?Colors.tealAccent:Colors.orangeAccent).withOpacity(selected ? .34 : .13),stroke=Paint()..style=PaintingStyle.stroke..strokeWidth=(selected?2.4:1)..color=(selected?Colors.white:Colors.white38);for(final ring in shape.rings){if(ring.length<3)continue;final path=Path();for(var i=0;i<ring.length;i++){final p=_geoToScreen(ring[i],size,bounds);if(i==0)path.moveTo(p.dx,p.dy);else path.lineTo(p.dx,p.dy);}path.close();canvas.drawPath(path,fill);canvas.drawPath(path,stroke);}if(selected){final cp=_geoToScreen(Offset(shape.lng,shape.lat),size,bounds);canvas.drawCircle(cp,4,Paint()..color=Colors.white);}}}@override bool shouldRepaint(covariant _ProvincePainter oldDelegate)=>true;}
...
class WorldPainter extends CustomPainter {final GameState game;WorldPainter(this.game);@override void paint(Canvas canvas,Size size){
  ...
  final grid=Paint()..color=Colors.white.withOpacity(.025)..strokeWidth=1;for(int i=1;i<10;i++){canvas.drawLine(Offset(size.width*i/10,0),Offset(size.width*i/10,size.height),grid);}for(int i=1;i<8;i++){canvas.drawLine(Offset(0,size.height*i/8),Offset(size.width,size.height*i/8),grid);}
  ...
  for(final c in game.countries.values){
    ...
    canvas.drawPath(path,Paint()..color=col.withOpacity(c.id==game.selected ? .22 : .09));canvas.drawPath(path,Paint()..color=col.withOpacity(.42)..style=PaintingStyle.stroke..strokeWidth=(c.id==game.selected?2:1));
  }
  for(final c in game.countries.values){for(final n in c.neighbors){final o=game.countries[n];if(o==null||c.id.compareTo(o.id)>0)continue;final hostile=c.controller!=o.controller;final mineFront=hostile&&(game.isMine(c.id)||game.isMine(o.id));final link=Paint()..color=(mineFront?Colors.redAccent:Colors.white.withOpacity(.10))..strokeWidth=(mineFront?3:1);canvas.drawLine(Offset(c.pos.dx*size.width,c.pos.dy*size.height),Offset(o.pos.dx*size.width,o.pos.dy*size.height),link);}}
  for(final c in game.countries.values){final p=Offset(c.pos.dx*size.width,c.pos.dy*size.height);final mine=game.isMine(c.id);final selected=c.id==game.selected;final online=c.controller.startsWith('P:')&&!mine;final paint=Paint()..color=mine?Colors.tealAccent:(online?Colors.purpleAccent:Colors.orangeAccent.withOpacity(.92));canvas.drawCircle(p,selected?10:7,paint);canvas.drawCircle(p,selected?15:11,Paint()..color=Colors.black.withOpacity(.2)..style=PaintingStyle.stroke..strokeWidth=1);if(selected){canvas.drawCircle(p,16,Paint()..color=Colors.white..style=PaintingStyle.stroke..strokeWidth=2);}if(mine&&!game.isSupplyConnected(c.id)){canvas.drawCircle(p,13,Paint()..color=Colors.redAccent..style=PaintingStyle.stroke..strokeWidth=2);}final tp=TextPainter(text:TextSpan(text:c.id,style:TextStyle(fontSize:9,color:Colors.white.withOpacity(.9),fontWeight:FontWeight.bold)),textDirection:TextDirection.ltr)..layout();tp.paint(canvas,p+const Offset(9,-5));
